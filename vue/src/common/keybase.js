...
async function queryKeybaseImages(keybaseImageRecords, onChunkReady) {
  return chunk(keybaseImageRecords, CHUNK_SIZE).reduce(async (all, keybaseImageRecords, index) => {
    const previous = await all
    if (index > 0) await new Promise((resolve) => setTimeout(resolve, CHUNK_THROTTLE))
...
export async function updateValidatorImages(currentValidators, onChunkReady) {
  ...
  const nonUpdateableValidators = currentValidators.filter(
    ({ identity }) => !updateableKeybaseHashes.find(({ keybaseHash }) => identity === keybaseHash),
  )
  ...
  await queryKeybaseImages(
    ...
    (updatedKeybaseHashes) => {
      ...
      onChunkReady(
        enrichedValidatorsWithPicture(currentValidators.filter(({ identity }) => updatedKeybaseHashes[identity])),
      )
    },
  )
