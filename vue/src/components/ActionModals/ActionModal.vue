  ...
  <transition v-if="show" name="slide-fade">
    <div v-focus-last class="action-modal" tabindex="0" @keyup.esc="close">
      ...
      <div v-else-if="step === feeStep" class="action-modal-form">
        <TableInvoice v-model="feeDenom" :amounts="amounts" :fees="networkFees.feeOptions" />
        <FormMessage
      ...
      <div v-else-if="step === signStep" class="action-modal-form">
        <form v-if="session.sessionType === SESSION_TYPES.LOCAL" @submit.prevent="validateChangeStep">
          <FormGroup class="action-modal-group" field-id="password" field-label="Password">
            <Field id="password" v-model="password" v-focus type="password" placeholder="Password" />
            <FormMessage v-if="$v.password.$error && $v.password.$invalid" name="Password" type="required" />
          </FormGroup>
        </form>
        <Card
          v-if="
      ...
      <div v-else-if="step === inclusionStep" class="action-modal-form">
        <Card icon="hourglass_empty" :spin="true">
          ...
          <div slot="subtitle">Waiting for confirmation from {{ network.name }}.</div>
        </Card>
      </div>
      ...
      <Card v-if="submissionError" class="form-msg sm form-msg--error submission-error">
      ...
      <div class="action-modal-footer">
        <slot name="action-modal-footer">
          <div v-if="[defaultStep, feeStep, signStep].includes(step)" class="action-modal-group">
            ...
            />
            <Button v-else-if="sending" value="Sending..." disabled="disabled" type="primary" />
            <Button
            ...
            />
            <Button v-else type="primary" value="Send" @click.native="validateChangeStep" />
          </div>
        </slot>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    steps() {
      const isExtensionSession = this.session.sessionType === SESSION_TYPES.KEPLR
      return ['Details', isExtensionSession ? undefined : 'Fees', 'Sign'].filter((x) => !!x)
    },
    isValidChildForm() {
  ...
  validations() {
    return {
      password: {
        required: requiredIf(() => this.session.sessionType === SESSION_TYPES.LOCAL && this.step === signStep),
      },
      invoiceTotal: {
        available: () =>
          this.amounts
            .concat(this.networkFees.feeOptions.find(({ denom }) => denom === this.feeDenom))
            .every(({ amount, denom }) => {
              const balance = this.balances.find((balance) => balance.denom === denom)
  ...
  methods: {
    ...
    previousStep() {
      switch (this.step) {
        case signStep:
          ...
          this.step = this.session.sessionType === SESSION_TYPES.KEPLR ? defaultStep : feeStep
          break
        case feeStep:
          this.step = defaultStep
          break
    ...
    async validateChangeStep() {
      ...
      switch (this.step) {
        case defaultStep:
          ...
          // Keplr is handling fees
          this.step = this.session.sessionType === SESSION_TYPES.KEPLR ? signStep : feeStep
          return
        case feeStep:
    ...
    async submit() {
      ...
      if (Object.entries(this.transactionData).length === 0 && this.transactionData.constructor === Object) {
      ...
      try {
        ...
        const { createSignBroadcast } = await import('~/signing/transaction-manager')

        // TODO currently not respected
        const HDPath = network.HDPath

        const block = await this.$store.dispatch('data/getBlock')
        const accountInfo = await this.$store.dispatch('data/getAccountInfo', this.session.address)
