<template>
  ...
  >
    <span v-if="session.sessionType === SESSION_TYPES.LEDGER" class="form-message withdraw-limit">
      Lunie will only withdraw rewards from 5 validators at a time because of a limitation with the Ledger&nbsp;Nano.
    </span>
    <FormGroup class="action-modal-form-group" field-id="amount" field-title="Amount">
      <div v-for="reward in totalRewards" :key="reward.denom" class="row">
        <Field :value="amountValue" :add-on="reward.denom" :is-disabled="true" />
      </div>
    </FormGroup>
  </ActionModal>
</template>
...
export default {
  name: `ClaimModal`,
  components: { Field, FormGroup, ActionModal },
  props: {
    address: {
      type: String,
      required: true,
  ...
  computed: {
    ...
    feeDenom() {
      ...
      if (this.balances && this.balances.length > 0) {
        ...
        const stakingDenomBalance = nonZeroBalances.find(({ denom }) => denom === this.stakingDenom)
    ...
    totalRewards() {
      ...
      return rewardsDenomArray.map(([denom, amount]) => ({ denom, amount })).sort((a, b) => b.amount - a.amount)
    },
    amountValue() {
      return fullDecimals(this.reward.amount)
    },
  },
  methods: {
    open() {
