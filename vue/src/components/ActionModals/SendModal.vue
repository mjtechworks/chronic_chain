<template>
  ...
  >
    ...
    >
      ...
      />
      <FormMessage v-if="$v.address.$error && !$v.address.required" name="Address" type="required" />
      <FormMessage
    ...
    >
      ...
      <FormMessage v-if="$v.amounts.$error && (!$v.amounts.required || amount === 0)" name="Amount" type="required" />
      <FormMessage v-else-if="$v.amounts.$error && !$v.amounts.decimal" name="Amount" type="numeric" />
      <FormMessage
        v-else-if="$v.amounts.$error && !$v.amounts.max"
        type="custom"
        :msg="`You don't have enough ${amounts.map(({ denom }) => denom)} to proceed.`"
      />
      <FormMessage v-else-if="$v.amounts.$error && !$v.amounts.min" :min="smallestAmount" name="Amount" type="min" />
      <FormMessage v-else-if="$v.amounts.$error && !$v.amounts.maxDecimals" name="Amount" type="maxDecimals" />
      <FormMessage
    ...
    <FormGroup
      ...
      field-label="Memo"
    >
      <Field id="memo" v-model="memo" type="text" @keyup.enter.native="enterPressed" />
      <FormMessage
...
import ActionModal from '@/components/ActionModals/ActionModal'
...
function availableDenoms(denoms, index, amounts) {
  return denoms.filter(
    ({ key: denom }) =>
      (amounts[index] && amounts[index].denom === denom) || !amounts.find((amount) => amount.denom === denom),
  )
}

export default {
  name: `SendModal`,
  components: { FormMessage, FormGroup, ActionModal, Field },
  filters: {
    availableDenoms,
  },
  props: {
  ...
  computed: {
    ...
    denomOptions(index) {
      return this.denoms ? this.denoms.map((denom) => ({ key: denom, value: denom })) : []
    },
    notifyMessage() {
      return {
  ...
  methods: {
    ...
    isMaxAmount(index) {
      ...
      if (selectedBalance.available === 0) {
        ...
        return parseFloat(this.amounts[index].amount) === selectedBalance.available
      }
    },
  ...
  validations() {
    return {
      ...
      amounts: {
        required: (x) => this.amounts.filter(({ amount }) => required(amount)).length === this.amounts.length,
        decimal: (x) => this.amounts.filter(({ amount }) => decimal(amount)).length === this.amounts.length,
        max: (x) =>
          this.amounts.filter((amount) => Number(amount.amount) <= this.getSelectedBalance(amount.denom).available)
            .length === this.amounts.length,
        min: (x) => this.amounts.filter(({ amount }) => Number(amount) >= SMALLEST).length === this.amounts.length,
        maxDecimals: (x) => {
          ...
          return amountsWithDecimals.length > 0
            ? amountsWithDecimals.filter(({ amount }) => Number(amount).toString().split('.')[1].length <= 6).length ===
                amountsWithDecimals.length
            : true
        },
      },
      denoms: { required },
