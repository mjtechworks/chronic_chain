...
export function getFees(transactionType, feeDenom) {
  ...
  const convertedFee = [
    {
      amount: BigNumber(fee.amount).div(coinLookup.chainToViewConversionFactor).toString(),
...
export async function createSignBroadcast({
  ...
  if (signingType === 'extension') {
    signedTx = await signWithExtension(messageType, message, transactionData, senderAddress, network)
  } else {
  ...
  const broadcastResult = await axios.post(`${network.apiURL}/txs`, broadcastBody).then((res) => res.data)
...
export async function pollTxInclusion(txHash, iteration = 0) {
  ...
  if (txFound) {
    ...
    throw new Error(`The transaction wasn't included in time. Check explorers for the transaction hash ${txHash}.`)
  }
}
