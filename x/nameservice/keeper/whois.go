...
func (k Keeper) GetWhois(ctx sdk.Context, key string) (types.Whois, error) {
	...
	k.cdc.MustUnmarshalLengthPrefixed(store.Get(byteKey), &whois)
	return whois, nil
}
...
func (k Keeper) SetWhois(ctx sdk.Context, name string, whois types.Whois) {
	...
	store := ctx.KVStore(k.storeKey)
	bz := k.cdc.MustMarshalLengthPrefixed(whois)
...
func listWhois(ctx sdk.Context, k Keeper) ([]byte, error) {
	...
	for ; iterator.Valid(); iterator.Next() {
		var whois types.Whois
		k.cdc.MustUnmarshalLengthPrefixed(store.Get(iterator.Key()), &whois)
